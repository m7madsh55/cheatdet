...
/**
 * return the quiz max allowed time in minutes
 *
 * @param int $quizid The ID for the quiz.
 * @return int The max time in minutes.
 */
function max_Time ($quizid)
{  global $DB;
$Time_limit= $DB->get_record('quiz',array ('id'=>$quizid));
$sec=$Time_limit-> timelimit;
$minute=$sec/60;
return $minute;
 }
/**
...
function getIP($userid,$quizid)
{
  ...
  foreach ($quiz_attempts as $attempt) { 
    $submissions[$user][$attempt->id] = $DB->get_record('logstore_standard_log' , array('action' =>  'submitted' , 'objectid' => $attempt->id));
        $ip[]= $submissions[$user][$attempt->id]->ip; 
        }
        return $ip;

}
function has_dupes($iplist,$userid) {
    $dupe_array = array();
    $duplicate = array();
    $userip=$iplist[$userid];
    $count=0;
